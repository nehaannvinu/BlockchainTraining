...
(async () => {
  ...
  let mintAddress = new web3.PublicKey(
    "JDd7RTEt8W3DY7D9Fm5Tkd4QUtG7Tp3XsCabGjPiVwmM"
  );
  ...
  let toTokenAccount = await tk.getOrCreateAssociatedAccountInfo(to.publicKey);

  console.log(
    "Token address of sender: " + fromTokenAccount.address.toBase58()
  );
  console.log(
    "Token address of receiver: " + toTokenAccount.address.toBase58()
  );
  ...
  let signatureUSer = nacl.sign.detached(transactionBuffer, to.secretKey);

  transaction.addSignature(from.publicKey, signature);

  transaction.addSignature(to.publicKey, signatureUSer);
